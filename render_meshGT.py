...
def render_pass(obj, objpath, texpath,fname):
    # change output image name to obj file name + texture name + random three
    # characters (upper lower alphabet and digits)
    fn = fname
    ...
    prepare_no_env_render_GT()
    links.remove(imglk)

    render_layers = tree.nodes.new('CompositorNodeRLayers')
    file_output_node_label = tree.nodes.new('CompositorNodeOutputFile')
    file_output_node_label.format.file_format = 'PNG'
    file_output_node_label.base_path = path_to_output_label
    file_output_node_label.file_slots[0].path = fn
    labellk = links.new(render_layers.outputs[0], file_output_node_label.inputs[0])
    scene.cycles.samples=256
    bpy.ops.render.render(write_still=False)
...
def render_pass_gt(obj, objpath, texpath,fname):
    # change output image name to obj file name + texture name + random three
    # characters (upper lower alphabet and digits)
    fn = fname
    ...
    file_output_node_gt.base_path = path_to_output_gt
...
def render_pass_aug(obj, objpath, texpath,fname):
    ...
    return fn
def render_img(objpath, texpath,fname):
   #add texture
